...
def get_filter(data):
        ...
        response = requests.post('http://db_iface:5000/save_twitter', headers=headers, json=p_data)
        return p_data
...
def get_watson(data):
        ...
        p_data = json.loads(response.text)
        response = requests.post('http://db_iface:5000/save_watson', headers=headers, json=p_data)
        return p_data
...
def post_filter(data, emotion):
        for item in data['features']:
                ...
                item['properties'] = {'text': prop['text'],
...
def handle():
        ...
        tweets = get_n_tweets(targs['q'],
                              targs['num'],
                              since_id=targs['since_id'],
                              max_id=targs['max_id'])
